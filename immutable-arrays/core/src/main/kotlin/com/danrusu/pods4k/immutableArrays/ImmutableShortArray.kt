...
public value class ImmutableShortArray @PublishedApi internal constructor(
     ...
     */
    @PublishedApi
    internal fun populateBitmap(bitmap: IntArray, predicate: (index: Int, element: Short) -> Boolean): Int {
        var numOneBits = 0
     ...
     */
    @PublishedApi
    internal fun select(bitmap: IntArray, numOneBits: Int): ImmutableShortArray {
        if (numOneBits == 0) return EMPTY
        if (numOneBits == size) return this

        val result = ShortArray(numOneBits)
        var resultIndex = 0
        var originalIndex = 0
