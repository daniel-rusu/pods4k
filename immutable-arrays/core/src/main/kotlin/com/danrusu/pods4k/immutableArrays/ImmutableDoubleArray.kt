...
public value class ImmutableDoubleArray @PublishedApi internal constructor(
    ...
    public inline fun filterIndexed(
        crossinline predicate: (index: Int, element: Double) -> Boolean,
     ...
     */
    @PublishedApi
    internal fun populateBitmap(bitmap: IntArray, predicate: (index: Int, element: Double) -> Boolean): Int {
        var numOneBits = 0
     ...
     */
    @PublishedApi
    internal fun select(bitmap: IntArray, numOneBits: Int): ImmutableDoubleArray {
        if (numOneBits == 0) return EMPTY
        if (numOneBits == size) return this

        val result = DoubleArray(numOneBits)
        var resultIndex = 0
        var bitmapIndex = 0
        var bitIndex = -1
     ...
     */
    public fun minWithOrNull(comparator: Comparator<in Double>): Double? {
     ...
     * @throws NoSuchElementException if this ImmutableDoubleArray is empty
     */
    public fun maxWith(comparator: Comparator<in Double>): Double {
        var maxElement = first()
        if (size == 1) return maxElement
     ...
     */
    public fun maxWithOrNull(comparator: Comparator<in Double>): Double? {
