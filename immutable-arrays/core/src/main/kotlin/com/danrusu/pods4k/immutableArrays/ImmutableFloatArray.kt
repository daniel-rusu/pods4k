...
public value class ImmutableFloatArray @PublishedApi internal constructor(
     ...
     */
    public inline operator fun iterator(): FloatIterator = values.iterator()
     ...
     */
    public fun chunked(size: Int): ImmutableArray<ImmutableFloatArray> {
    ...
    ): ImmutableArray<ImmutableFloatArray> {
     ...
     */
    @PublishedApi
    internal fun select(selection: Selection): ImmutableFloatArray {
    ...
    public fun partition(predicate: (element: Float) -> Boolean): Pair<ImmutableFloatArray, ImmutableFloatArray> {
        ...
        val first = FloatArray(numSelectedElements)
        var firstIndex = 0
        selection.forEachSelectedIndex { originalIndex ->
            first[firstIndex++] = values[originalIndex]
        }

        val second = FloatArray(size - numSelectedElements)
        var secondIndex = 0
        selection.forEachUnselectedIndex { originalIndex ->
            second[secondIndex++] = values[originalIndex]
        }
        return Pair(ImmutableFloatArray(first), ImmutableFloatArray(second))
    }
     ...
     */
    public fun minWithOrNull(comparator: Comparator<in Float>): Float? {
     ...
     */
    public fun maxWithOrNull(comparator: Comparator<in Float>): Float? {
