...
public value class ImmutableByteArray @PublishedApi internal constructor(
     ...
     */
    @PublishedApi
    internal fun populateBitmap(bitmap: IntArray, predicate: (index: Int, element: Byte) -> Boolean): Int {
        var numOneBits = 0
     ...
     */
    @PublishedApi
    internal fun select(bitmap: IntArray, numOneBits: Int): ImmutableByteArray {
        if (numOneBits == 0) return EMPTY
        if (numOneBits == size) return this

        val result = ByteArray(numOneBits)
        var resultIndex = 0
        var originalIndex = 0
