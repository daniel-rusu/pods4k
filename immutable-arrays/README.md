...
Mutation attempts result in immediate IDE errors and fail to compile:
...
Unlike read-only lists, immutable arrays cannot be mutated through casting allowing them to be safely shared without
...
When storing one of the 8 base types, immutable arrays use between **3 to 24 times less memory** than lists in typical
JVM environments:
