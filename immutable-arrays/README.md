...
Immutable Arrays are a safer and more efficient alternative to read-only lists while maintaining the same look and feel.
They are inline classes that compile to regular arrays in the generated bytecode while eliminating mutating abilities
and replacing common operations with highly-optimized versions. This reduces memory consumption and improves performance
quite significantly.
...
* **Fast**: 2 to 8X faster [benchmark results](BENCHMARKS.md) than lists for most operations!
* **Memory Efficient**: Over 4X [memory reduction](#memory-consumption) in most scenarios!
* **Type Safety**: Prevents accidental mutation attempts at compile time.

Ideal for memory-constrained environments, performance-critical workloads, or for ensuring data integrity. Immutable
Arrays are great for Android and backend JVM applications.

## Performance

Immutable Arrays are 2 to 8X faster than lists for most operations with some much faster. Array operations were replaced
with highly-optimized versions that preserve immutability and often outperform regular arrays. Immutability also enables
...
Here's a sample operation where we code naturally and Immutable Arrays automatically use primitives:
...
Most operations on regular arrays produce lists. Here's the same example with a regular primitive array and the
resulting list memory layout:

![Memory Layout of Read-only Lists](./resources/memory/list-memory-layout.drawio.png)

Note that the `values` variable references an `ArrayList`, which references an over-sized array, which itself references
wrapper objects that finally store the primitive values.

Classes that operate on generics, such as lists, can't store primitives directly. Primitives get auto-boxed into wrapper
objects and references to those wrappers are passed to the list. These wrapper objects are allocated in different
regions of memory depending on availability, and the garbage collector periodically moves surviving objects around, so
we can end up with the objects scattered throughout the heap.

Here's a more complex example showing how Immutable Arrays automatically switch from reference types to primitives:

```kotlin
// Automatically switches from an ImmutableArray<Person> to an ImmutableFloatArray
people.map { it.weightKg } // weightKg is a Float
```

</details>

Auto-boxing with lists increases memory consumption, adds an extra layer of indirection, reduces cache locality, and
adds extra pressure on the garbage collector which can affect the responsiveness of the application.
...
new collections. Additionally, Immutable Arrays are perfectly sized whereas lists end up with 17% unused capacity on
average when the final size isn't known in advance.
