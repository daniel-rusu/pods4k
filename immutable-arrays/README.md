...
Since Immutable Arrays compile to regular arrays in the generated bytecode, we can check if two "instances" represent
the same immutable array by checking if they reference the same backing array:

```kotlin
val first = immutableArrayOf("one", "two")
val second = immutableArrayOf("one", "two")

// true since they contain identical values
first == second

// false as they were created separately
first.referencesSameBackingArrayAs(second)

// Immutability allows us to safely share instances behind the scenes reducing memory consumption
first.take(100).referencesSameBackingArrayAs(first) // true
first.filter { it.isNotEmpty() }.referencesSameBackingArrayAs(first) // true
// etc.
```

We can think of Immutable Arrays as a kind of virtual quantum particle that comes in and out of existence (see
Auto-boxing above). Since immutable arrays aren't persistent wrapper objects, attempting to use their identities is not
supported. Here are
