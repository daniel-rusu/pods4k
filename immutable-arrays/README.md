...
* **Clean & Familiar**
    * Uses a list-like syntax providing easy adoption with clean code.
* **True Immutability**
...
* **Fast**
    ...
    * Immutability unlocked hundreds of optimizations.
* **Memory Efficient**
    * Over 5X memory reduction versus lists for many common scenarios!
        * E.g. `people.map { it.weightKg }` when `weightKg` is one of the 8 base types such as a `Float`.
    * Immutability allows operations to retain zero extra memory in hundreds of scenarios.
        * E.g. `people.filter { it.isEmployed() }` returns same instance when everyone is employed.
* **Type Safety**
...
regular arrays in the generated bytecode. This is because Immutable Arrays also replace the array operations with highly
optimized versions. See the [Benchmarks page](BENCHMARKS.md) for detailed explanations.
...
Regular equality work as expected the same as we use lists. For referential equality, the Kotlin compiler prevents using
`===` because Immutable Arrays are inline classes that get erased at compile time. They compile to regular arrays in the
generated bytecode so they don't have the traditional object identity since the wrapper object is erased.

Use `referencesSameArrayAs` to check whether two variables reference the same array instance:
...
names.referencesSameArrayAs(sameNames)
