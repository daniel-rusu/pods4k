...
Note that all benchmarks use like-for-like comparisons with regular arrays (eg. `ImmutableFloatArray` vs.
`FloatArray`). The [Benchmarks page](BENCHMARKS.md) also includes explanations of why Immutable Arrays achieve seemingly
impossible results even though they compile to regular arrays in the generated bytecode.
