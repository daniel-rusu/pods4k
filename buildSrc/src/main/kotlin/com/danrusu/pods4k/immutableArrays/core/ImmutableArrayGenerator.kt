...
private fun TypeSpec.Builder.addReferencesSameArrayAs(baseType: BaseType) {
    ...
    function(
        kdoc = """
            Immutable Arrays compile to regular arrays in the generated bytecode so this checks if both immutable arrays reference the same array instance.

            Kotlin doesn't allow checking reference equality of inline-class "instances" since auto-boxing the same immutable array twice can result in two separate wrapper objects that both represent the same immutable array with both wrappers referencing the same backing array instance.  This method allows us to safely check if we're dealing with the same immutable array bypassing the auto-boxing concerns.
        """.trimIndent(),
